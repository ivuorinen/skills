...
# A store this tool already owns, recognised by files only it writes.
_STORE_MARKERS = ("INDEX.md", "resolved.jsonl")
...
def _refuse_project_root(root: Path) -> None:
    ...
    if not root.is_dir():
        return
    if any((root / marker).exists() for marker in _STORE_MARKERS):
        return
