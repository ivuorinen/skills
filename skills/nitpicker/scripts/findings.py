...
# repository root can hold for its own reasons, and which `write_index` would
# then replace.
_INDEX_SIGNATURE = "Generated by `findings.py index` — do not edit by hand."
# A directory a person would call "the project", not "the store".
_PROJECT_MARKERS = (".git", "pyproject.toml", "package.json", "Makefile")


def _owns_store(root: Path) -> bool:
    """True when `root` holds this tool's ledger or an INDEX.md this tool wrote.

    Ceiling: the signature is text anyone can copy into a file, so this tells a
    store from a stray `INDEX.md`, not from a deliberate forgery.
    """
    if (root / "resolved.jsonl").exists():
        return True
    try:
        return _INDEX_SIGNATURE in (root / "INDEX.md").read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError):
        return False


def _refuse_project_root(root: Path) -> None:
    """Refuse to build a findings store in what is plainly a repository root.

    `--root` is the store directory, and its name invites the other reading:
    three separate command runs passed a project root in one session, each
    scattering `<repo>/<auditor>/open/*` and an `INDEX.md` across the working
    tree and replacing the repository's own `.gitattributes` and `.gitignore`
    with the store's (audit-141e9b71). The damage outlived the run that caused
    it, because the run that repaired it was not the run that repeated it.

    An existing store is always allowed — it is recognised by its own files, so
    a store that legitimately sits beside a marker keeps working — and so is a
    store directory that does not exist yet, which is the first-finding case in
    a fresh repository.
    """
    if not root.is_dir():
        return
    if _owns_store(root):
        return
    found = [marker for marker in _PROJECT_MARKERS if (root / marker).exists()]
    if found:
        raise FindingError(
            f"refusing to create a findings store in {root}: it looks like a project "
            f"root ({', '.join(found)}). --root is the store directory, not the "
            f"repository — pass {DEFAULT_ROOT} or another dedicated path."
        )


def overlapping_open_findings(
    root: Path, auditor: str, area: str, filed: Path | None = None
) -> list[str]:
    """Warning lines for open findings another auditor already filed on `area`.

    Commands filed on the same symbols in one session, and another dropped
    duplicates of its own by reading the store itself, because nothing surfaced
    the overlap at filing time.

    Advisory, never blocking, and never deduplicating on the agent's behalf: one
    area legitimately carries findings from several lenses — a path traversal and
    a dead parameter in one file are two defects, not one filed twice. The reader
    decides; this only makes the overlap visible at the moment it is created.
    """
    try:
        others = [
            (fm.get("id", path.stem), fm.get("auditor", "?"), title)
            for path, fm, title in iter_open(root)
            if path != filed and fm.get("auditor") != auditor and fm.get("area") == area
        ]
    except OSError:
        # The store was readable enough to file into a moment ago; a race with a
        # concurrent run must not turn a successful filing into a failure.
        return []
    if not others:
        return []
    lines = [f"[note] {area} already has {len(others)} open finding(s) from another auditor:"]
    lines += [f"         {fid} ({who}) {title}" for fid, who, title in sorted(others)]
    lines.append("       Check whether this is the same defect before leaving both open.")
    return lines
...
def clear_baseline(root: Path) -> bool:
    """Delete the baseline, under the same guards as every other store mutation.

    A bare `unlink` followed a committed `docs/audit/findings` symlink and deleted
    `baseline.json` wherever it pointed (security-3fbe26a3's class). The symlink
    check runs first so an absent baseline behind a link is refused too, and the
    lock is taken only when there is something to delete, so a no-op clear does
    not create the store directory.
    """
    p = baseline_path(root)
    _refuse_symlink(root, p)
    if not p.exists():
...
def ensure_store_gitattributes(root: Path) -> None:
    ...
    try:
        ...
        gitattributes = root / _STORE_GITATTRIBUTES
        # Write only into a file this tool owns. A .gitattributes that exists
        # without the mark belongs to whoever put it there, and overwriting it is
        # exactly how a mis-rooted run replaced a repository's own copy
        # (audit-141e9b71). Leaving it costs the linguist-generated mark, which
        # check_review_hygiene already warns about — a warning beats a clobber.
        if not gitattributes.exists():
            _atomic_write(gitattributes, _STORE_GITATTRIBUTES_BODY)
...
def main(argv: list[str] | None = None) -> int:  # noqa: C901
    ...
    def add_root(p: argparse.ArgumentParser) -> None:
        p.add_argument(
            ...
            help=(
                f"the findings store directory, not the repository "
                f"(default: {DEFAULT_ROOT}); pointing this at a project root is "
                f"refused"
            ),
        )
    ...
    add_root(p_new)
    p_new.add_argument("--auditor", required=True, help="the filing command's own name")
    p_new.add_argument(
        "--severity", required=True, choices=SEVERITIES, help="risk level, not preference"
    )
    p_new.add_argument("--category", required=True, choices=CATEGORIES, help="finding category")
    p_new.add_argument("--area", required=True, help="the file, directory or subsystem")
    ...
    add_root(p_res)
    p_res.add_argument("id", help="the finding id to resolve")
    p_res.add_argument(
        "--status",
        required=True,
        choices=("fixed", "invalid"),
        help="fixed: the defect is gone; invalid: the finding was wrong",
    )
    p_res.add_argument("--notes", required=True, help="what changed, or why it was invalid")
    p_res.add_argument("--date", default=None, help="ISO date to record instead of today")
    p_res.add_argument(
    ...
    add_root(p_list)
    p_list.add_argument("--auditor", default=None, help="only findings under this auditor key")
    p_list.add_argument(
        "--status", default=None, choices=STATUSES, help="only findings in this state"
    )
    ...
    p_exp.add_argument(
        "--format", required=True, metavar="{sarif,json,junit}", help="output format"
    )
    p_exp.add_argument("--auditor", default=None, help="only findings under this auditor key")
    p_exp.add_argument(
        "--severity", default=None, choices=SEVERITIES, help="only findings of this severity"
    )
    p_exp.add_argument(
    ...
    add_root(p_show)
    p_show.add_argument("id", help="the finding id to print")

    p_val = sub.add_parser("validate", help="validate the store or given files")
    add_root(p_val)
    p_val.add_argument(
        "paths", nargs="*", type=Path, help="finding files to check; default: the whole store"
    )
    ...
    add_root(p_mig)
    p_mig.add_argument(
        "sources", nargs="+", type=Path, help="v1 docs/audit/*-findings.md files to convert"
    )
    ...
    if args.cmd == "new":
        ...
        for line in overlapping_open_findings(args.root, args.auditor, args.area, path):
    ...
    if args.cmd == "baseline":
        if args.clear:
            try:
                cleared = clear_baseline(args.root)
            except FindingError as e:
                print(f"ERROR  {e}", file=sys.stderr)
                return 1
