...
7. **Cross-package / external consumers** — a monorepo sibling or a published-package importer that this repo cannot see.
...
2. **Rule out reachability for each candidate.** For every candidate symbol or file, run all seven channels of the Reachability rule repo-wide. A candidate that survives one unchecked channel is not yet dead. Record which channels were checked and their results.
...
- Deleting on a grep-for-name negative alone, before the other six channels are checked
