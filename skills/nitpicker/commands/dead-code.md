...
4. **File findings** via the store protocol in `_conventions.md`, under the `dead-code` auditor key (category `maintainability`; an `unreachable-code` finding that masks a bug is `correctness`). Route each out-of-scope defect as a one-line pointer to its command.
