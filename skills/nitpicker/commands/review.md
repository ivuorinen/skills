...
8. **Time, dates, and time zones** — naive and timezone-aware datetimes mixed; UTC-vs-local confusion at a storage or display boundary; DST gaps and overlaps (a local time that never occurs or occurs twice); wall-clock used for a duration where a monotonic clock is required; clock skew assumed away across services; epoch unit confusion (seconds vs milliseconds) and 2038 overflow; date math that adds 24h instead of one calendar day across a DST change; locale-dependent parse/format; leap years and leap seconds
9. **Money and precision** — floating point (`float`/`double`) for currency instead of integer minor units or a decimal type; rounding mode unspecified or wrong (half-up vs banker's), or rounding applied too early or too late; amounts in different currencies combined without conversion; unit mismatch (dollars vs cents); truncation on integer division of an amount; accumulation error summing floats in a loop; precision lost across a serialization or DB numeric boundary
