...
It reads every instruction file the detected harnesses load — both roles in the Harness scope table — and reports three defects that only exist across files or against a whole-set total: `instruction_budget` (the set's directives against the ~150 a session can carry once the harness takes its share), `position_risk` (a critical rule titled in the middle 20–80% of a root file, where it is skimmed past), and `cross_file_duplicate` (one directive stated in two files). Only the budget limit blocks. It names the harnesses it detected in `harnesses`.
