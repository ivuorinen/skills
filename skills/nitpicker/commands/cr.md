...
The termination signals come from **direct API polling**, not from the fetch batch alone: read the summary text and its `updated_at` (the fetcher returns `summary_comments[].updated_at`, the baseline for the rate-limit wait in step 3), and read a review's `state`/`commit_id` from the PR **reviews API** (`gh pr view --json reviews`). An empty `APPROVED` review carries no body, so it is absent from `review_bodies` — detect approval from the reviews API state, never by expecting it in the fetch output.
