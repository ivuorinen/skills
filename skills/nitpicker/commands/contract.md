...
4. **File findings** via the store protocol in `_conventions.md`, under the `contract` auditor key. Every finding carries the four-part evidence: the declared element (file:line), the implementation (file:line or "absent"), the mismatch, and the consumer-visible consequence. Axis 2 findings add the breaking/non-breaking classification vs the declared bump; cross-reference overlapping `/nitpicker commits` finding ids in the mismatch.
