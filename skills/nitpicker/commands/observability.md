...
5. **File findings** via the store protocol in `_conventions.md`, under the `observability` auditor key. The finding body records the class, what the path emits today ("nothing" counts), what diagnosing it needs, and the 3am scenario; the Fix is the exact emission to add or correct — never a business-logic change.
