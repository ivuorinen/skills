...
| Findings history | Every finding in the store (`np_list_findings`, else `python3 "${CLAUDE_SKILL_DIR}/scripts/findings.py" list` — all auditors and statuses; non-Claude agents resolve the path relative to this skill's directory) plus any legacy `docs/audit/*-findings.md` files. A defect class that recurs — or that a fixed entry shows was hand-fixed — is a recurring, hook-preventable candidate |
