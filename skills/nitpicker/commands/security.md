...
7. Re-validate every open finding per `_conventions.md`, using the match keys from step 5 (for SAST/gosec, ignore line-number drift of ±10). Identify which tools ran successfully this pass (probed found AND did not error). A finding whose detecting tool did not run is re-validated against the source by reading, as any other lens would: resolve it only on evidence, and where the source cannot settle it, leave it open, record it as `unverified` — never `clean` — and emit: "Unverified: N finding(s) whose detecting tool did not run this pass: <list>."
