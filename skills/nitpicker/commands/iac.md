...
| **missing-limits** | k8s container with no `resources.limits`/`requests` (a noisy neighbor or OOM starves the node); no `livenessProbe`/`readinessProbe`; a Dockerfile for a long-running service with no `HEALTHCHECK` and no external health check covering it (a k8s `livenessProbe`/`readinessProbe` or a Compose `healthcheck`), so a hung container goes undetected — a build-only, CI, or one-shot image is exempt; a Compose service with no restart policy or resource cap | Set CPU/memory `requests` and `limits`; add liveness/readiness probes; add a `HEALTHCHECK`; set a bounded restart policy |
