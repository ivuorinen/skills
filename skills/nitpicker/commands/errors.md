...
3. **File findings** via the store protocol in `_conventions.md`, under the `errors` auditor key (category `reliability`). Each finding's Evidence carries the quoted handler code, the scenario (the concrete upstream failure), and the consequence; the Fix is the exact error-path change — the happy path stays untouched.
...
- **"I'll grep for 'catch' and call that the audit."** Several defect classes need no catch block at all: ignored-error-signal (including unawaited calls), silent-retry, masking-fallback. Enumerate every surface category in step 1; a catch-only sweep is an INCOMPLETE run misreported as complete.
