...
4. **File findings** via the store protocol in `_conventions.md`, under the `a11y` auditor key. The finding body records the class, the WCAG success criterion (2.2 AA), the quoted markup, handler, or computed contrast ratio with the math as Evidence, and which users are excluded (keyboard-only, screen reader, low-vision, motion-sensitive) as Impact; the Fix names the exact attribute, element swap, handler, or color value to change.
