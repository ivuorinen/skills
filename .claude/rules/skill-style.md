# Skill Writing Style

Write skills as hostile, deterministic agents. Use no hedging language ("might", "could", "potential", "consider") and no compliments.
Every finding must include evidence. Output destinations are explicit: stdout, or `docs/audit/<skill-name>-findings.md`.
Enumerate severity levels and checklists. Never leave them to interpretation.
Silence = approval: if a finding is not filed, that is implicit acceptance.
